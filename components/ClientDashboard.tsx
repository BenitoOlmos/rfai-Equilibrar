                ...
                <div className="w-[calc(100%-3rem)] md:w-[calc(50%-2.5rem)] ml-4 md:ml-0 p-5 md:p-6 bg-white rounded-2xl md:rounded-3xl shadow-sm border border-slate-100 transition-all hover:shadow-lg">
                  ...
                  {!isLocked && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-3">
                      <button onClick={() => setShowTest(true)} className={`flex items-center gap-3 p-3 md:p-3 rounded-xl bg-slate-50 border border-slate-100 transition-all group/btn text-left active:scale-[0.98] ${user.program === 'ANGUSTIA' ? 'hover:bg-indigo-50 hover:border-indigo-200' : 'hover:bg-brand-50 hover:border-brand-200'}`}>
                        <div className={`w-10 h-10 md:w-8 md:h-8 rounded-full bg-white flex items-center justify-center shadow-sm shrink-0 ${user.program === 'ANGUSTIA' ? 'text-indigo-600' : 'text-brand-600'}`}><FileText size={18} /></div>
                        <div><span className="block text-sm font-bold text-slate-700">Test</span><span className="text-[10px] text-slate-400 block md:hidden sm:inline">Evaluar estado</span></div>
                      </button>
                      ...
                      {config.hasMeet && (
                        <div className="relative group/meet">
                          ...
                          <a href="https://meet.google.com" target="_blank" rel="noreferrer" className="flex items-center gap-3 p-3 md:p-3 rounded-xl bg-slate-50 hover:bg-slate-100 border border-slate-100 hover:border-slate-200 transition-all group/btn text-left relative overflow-hidden h-full active:scale-[0.98]">
                            <div className="w-10 h-10 md:w-8 md:h-8 shrink-0 rounded-full bg-white flex items-center justify-center text-slate-600 shadow-sm z-10"><Video size={18} /></div>
                      ...
                      <button onClick={() => setShowGuide(true)} className="flex items-center gap-3 p-3 md:p-3 rounded-xl bg-slate-50 hover:bg-amber-50 border border-slate-100 hover:border-amber-200 transition-all group/btn text-left active:scale-[0.98]">
                        <div className="w-10 h-10 md:w-8 md:h-8 rounded-full bg-white flex items-center justify-center text-amber-500 shadow-sm shrink-0"><FileText size={18} /></div>
                        <div><span className="block text-sm font-bold text-slate-700">Guía</span><span className="text-[10px] text-slate-400 block md:hidden sm:inline">Trabajo personal</span></div>
                      </button>
